<script>
    ...
    <div class="w-full px-7 lg:px-12  mt-10 bg-transparent lg:mt-20 xl-1:px-0 lg:pb-16 xl-1:pb-0">
        <div class="w-full flex flex-col xl-1:max-w-[80rem] xl-1:m-auto">
            {#if isLoading}
                <CustomLoader/>
            {:else}
                <div class="w-full flex flex-col">
                    <button class="w-[6.5rem]  bg-white dark:bg-darkBlue flex flex-row justify-between items-center pl-6 pr-[1.44rem] pb-[0.37rem] pt-[0.38rem] shadow-customXL lg:shadow-none border-0 outline-0 rounded-sm" on:click={() => goto('/')}>
                        <img src={left_arrow_icon} loading="lazy" alt="left_arrow_icon" class={$isDarkMode && "whiteFilter"}>
                        <span class="text-darkBlueMD dark:text-white font-light text-sm">Back</span>
                    </button>
                    ...
                    <div class="mt-16 flex flex-col lg:flex-row lg:justify-between lg:items-center">
                        ...
                        <div class="mt-11 lg:mt-0 text-darkBlueMD dark:text-white lg:w-[28.375rem] xl:w-[34.375rem] xl-1:w-[37.375rem]">
                            ...
                            {#if $countryNameData?.data?.data[0]?.borders}
                                <div class=" flex flex-col mt-[2.13rem] mb-16 lg:mb-0 xl:flex-row ">
                                    <h3 class="text-base font-semibold w-[9rem] block">Border Countries:</h3>
                                    <div class="mt-4 flex flex-row gap-[0.63rem] flex-wrap xl:flex-1 xl:mt-0 empty:hidden">
                                        {#each $countryNameData?.data?.data[0]?.borders as items}
                                            <div class="rounded-sm pl-[1.88rem] pr-[1.87rem] py-[0.38rem] bg-white dark:bg-darkBlue text-xs shadow-customXS">
                                                {items}
                                            </div>
                                        {/each}
